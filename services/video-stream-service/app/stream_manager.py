...
class StreamManager:
    ...
    def start_stream(
        ...
        thread=threading.Thread( #her kameraya ayrı parçacık açılır
            target=self._worker, 
            args=(info,), 
            daemon=True,
            ...
            while not info._stop_event.is_set():
                ...
                if not ret:
                    if isinstance(source, str) and source.endswith(".mp4"):
                        ...
                        break  # video bitti, dur
                    else:
                        # RTSP kopması, yeniden bağlan
                ...
                if frame_index % settings.FRAME_SKIP !=0:   #her n. frame'ı işle
                    continue
    ...
    def _send_to_inference(
        ...
        try:
            ...
            if not success:
                logger.warning(f"[INFERENCE] Frame encode edilemedi → drone_id={info.drone_id}")
                return None
            ...
            detections=[
                ...
                if d["confidence"]>= info.confidence_threshold
            ]

            logger.info(
                f"[INFER] {info.drone_id} | model={info.model_name} "
